...
public final class AIOptimizationStorage: Sendable {
    ...
    public static let defaultPromptTemplate = """
        ...
        2. 语境参考：利用 <history> 理解当前对话的语境，推断代词的指代，解决同音词歧义。绝对不要将历史记录合并到最终输出中。
        3. 去除冗余：精准删除无意义的口水词和口吃造成的重复字词。
        4. 英文与术语纠错：修复中英夹杂时的同音字、近音字或单个字母的识别错误（结合语境判断，例如判断"K"是代表"Key"还是普通的"OK"）。
        5. 句式与标点：还原口语倒装句的正常语序，并根据语气补全或修正标点符号。
        6. 最高原则：保持原意，绝对不能改变用户的核心意思，不要擅自总结、缩写或添加内容。
        </guidelines>
